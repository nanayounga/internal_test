			...
			<div class="panel-body">
				<div class="form-group {{ $errors->has('user_name') ? 'has-error' : '' }}">
					<div class="col-md-4" style="margin-bottom: 10px;">
						<input class="form-control" name="user_name" text="text" id="search_text" value="{{ old('user_name') }}" maxlength="20" placeholder="ID、名前、Email、部署を入力" required>
		...
		else
		{
			...
			for (var i = 0; i < data.length; i++)
			{
				list.append($('<tr></tr>')
					.append($('<td></td>')
						.append($('<a></a>')
							.attr('href','{{ url("user/show/") }}/'+ data[i]["user_id"])
					...
					.append($('<td></td>')
						.append($('<a></a>')
							.attr('href','{{ url("department/show/") }}/'+ data[i]["department_id"])
